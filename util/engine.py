...
def train_one_epoch_enhance_acc_pre(
    ...
    for i in range(len(data_loader)):
        with accelerator.accumulate(model):
            ...
            accelerator.backward(losses_sigma, retain_graph=True)
...
def train_one_epoch_enhance_acc(
    ...
    for i in range(len(data_loader)):
        ...
        # collect logging messages
        training_logs = {"loss": losses_reduced.item(), "loss_estimation": losses_sigma_reduced.item(),
                         **loss_dict_reduced}
        training_logs.update({**loss_dict_sigma_reduced})
        training_logs.update({"lr": optimizer.param_groups[0]["lr"]})
        metric_logger.update(**training_logs)
