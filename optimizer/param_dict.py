...
def finetune_backbone_and_linear_projection(model, lr):
    ...
    for name, parameters in model.named_parameters():
        if not parameters.requires_grad or "noise_layer" in name:
            continue
        # if match_name_keywords(name, "hyper_module"):
        #     hyper_module.append(parameters)
        #     continue
        if (
...
def finetune_backbone_and_linear_projection_modify(model, lr):
    linear_keywords = ("reference_points", "sampling_offsets")
    norm_bias_keywords = ("norm", "bias")
    backbone = []
    backbone_norm = []
