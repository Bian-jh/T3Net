...
from util.engine import evaluate_acc, train_one_epoch_enhance_acc_pre
...
def parse_args():
    parser = argparse.ArgumentParser(description="Enhance the detector")
    parser.add_argument("--config-file", default="configs/enhance_config_aitodv2_pre.py")
    parser.add_argument(
        "--mixed-precision",
        type=str,
        default=None,
...
def train():
    ...
    optimizer_noise = torch.optim.AdamW(model.transformer.encoder.noise_layer.parameters(), lr=1e-5)
    ...
    if weight_path is not None and os.path.isfile(weight_path):
        ...
        #     if "alpha" in k or "level_embeds" in k or "enc_mask_predictor" in k or "enc_output" in k:
        #         new_k = 'transformer.' + k
    ...
    model, optimizer_noise, optimizer, train_loader, test_loader, lr_scheduler = accelerator.prepare(
        model, optimizer_noise, optimizer, train_loader, test_loader, lr_scheduler
    )
    ...
    for epoch in range(cfg.starting_epoch, cfg.num_epochs):
        train_one_epoch_enhance_acc_pre(
            model=model,
        ...
        if (epoch + 1) > 9 or epoch == 0:
            logger.info("Start evaluation")
            coco_evaluator = evaluate_acc(model, test_loader, epoch, accelerator)

            # save best results
            print(coco_evaluator.coco_eval["bbox"].stats[:8])
            cur_ap, _, cur_ap50 = coco_evaluator.coco_eval["bbox"].stats[:3]
