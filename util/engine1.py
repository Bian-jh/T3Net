...
def train_one_epoch_enhance_acc(
    model, optimizer_noise, optimizer, data_loader, epoch, print_freq=50, max_grad_norm=-1, accelerator=None
):
    logger = logging.getLogger(os.path.basename(os.getcwd()) + "." + __name__)
    model.train()
    # BatchNorm and Dropout should change into eval mode
    # model.module.backbone.eval()
    # model.module.transformer.encoder.layers.eval()
    # model.backbone.eval()
    # model.transformer.encoder.layers.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter("lr", utils.SmoothedValue(window_size=1, fmt="{value:.6f}"))
    metric_logger.add_meter("data_time", utils.SmoothedValue(fmt="{avg:.4f}"))
    metric_logger.add_meter("iter_time", utils.SmoothedValue(fmt="{avg:.4f}"))
    ...
    for i in range(len(data_loader)):
        with accelerator.accumulate(model):
            # model forward
            loss_dict, loss_dict_sigma = model(images, targets)
            losses = sum(loss for loss in loss_dict.values())
            losses_sigma = sum(loss for loss in loss_dict_sigma.values())
            ...
            # backward propagation
            optimizer.zero_grad()
            accelerator.backward(losses, retain_graph=True)
            if accelerator.sync_gradients and max_grad_norm > 0:
                accelerator.clip_grad_norm_(model.parameters(), max_grad_norm)
            optimizer.step()

            ori = [p.clone() for n, p in model.transformer.encoder.named_parameters()]
            ori_name = [n for n, p in model.transformer.encoder.named_parameters()]

            optimizer_noise.zero_grad()
            accelerator.backward(losses_sigma)
            optimizer_noise.step()

            after = [p.clone() for p in model.transformer.encoder.parameters()]
            for name, p1, p2 in zip(ori_name, ori, after):
                print(name, torch.norm(p1-p2))
        ...
        with torch.no_grad():
            ...
            loss_dict_sigma_reduced = accelerator.reduce(loss_dict_sigma, reduction="mean")
            losses_sigma_reduced = sum(loss for loss in loss_dict_sigma_reduced.values())
            loss_sigma_value = losses_sigma_reduced.item()
        ...
        if not math.isfinite(loss_sigma_value):
            logger.warning(f"Loss is {loss_sigma_value}, stopping training")
            logger.warning(loss_dict_sigma_reduced)
            sys.exit(1)

        # collect logging messages
        training_logs = {"loss": losses_reduced.item(), "loss_estimation": losses_sigma_reduced.item(), **loss_dict_reduced}
        training_logs.update({**loss_dict_sigma_reduced})
        training_logs.update({"lr": optimizer.param_groups[0]["lr"]})
        metric_logger.update(**training_logs)
...
def evaluate_acc(model, data_loader, epoch, accelerator=None):
    ...
    metric_names = ["AP", "AP@25", "AP@50", "AP@75", "AP-vt", "AP-t", "AP-s", "AP-m"]
    metric_names += ["_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_"]
