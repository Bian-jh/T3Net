...
output_dir = '/t3net_visdrone_convnextb'
find_unused_parameters = False  # useful for debugging distributed training

# define dataset for train
coco_path = '/data/VisDrone-DET2019' # /PATH/TO/YOUR/COCODIR
train_transform = presets.detr  # see transforms/presets to choose a transform
train_dataset = CocoDetection(
    img_folder=f"{coco_path}/VisDrone2019-DET-train/images",
    ann_file=f"{coco_path}/VisDrone2019-DET-train/visdrone_train_coco.json",
    transforms=train_transform,
    train=True,
)
test_dataset = CocoDetection(
    img_folder=f"{coco_path}/VisDrone2019-DET-val/images",
    ann_file=f"{coco_path}/VisDrone2019-DET-val/visdrone_val_coco.json",
    transforms=None,  # the eval_transform is integrated in the model
)


# model config to train
model_path = "configs/t3net/enhance_convnextb_visdrone.py"
...
resume_from_checkpoint = "/t3net_visdrone_pre_convnextb/best_ap.pth"
# resume_from_checkpoint = None

learning_rate = 1e-4  # initial learning rate
optimizer = optim.AdamW(lr=learning_rate, weight_decay=1e-4, betas=(0.9, 0.999))
lr_scheduler = optim.lr_scheduler.MultiStepLR(milestones=[7, 10], gamma=0.1)
